...
from helper_functions import fmt4, contiguous_regions, clean_boolean_mask, robust_snr, adaptive_smooth, load_field
...
def plot_single_linecut(params, T, rho, candidates) -> None:
    ...
    # Derived curves
    rho_smoothed = adaptive_smooth(rho, T)
    rho_smoothed_hampel = adaptive_smooth(hampel(rho).filtered_data, T)
    ...
    ax2.plot(T, hampel(rho).filtered_data, marker='o', markersize=3, markerfacecolor='none', markeredgecolor='navy',linewidth=1.0, color='blue')

    ax3.set_title("Smoothed Data + Hampel")
    ax3.plot(T, rho_smoothed_hampel, marker='o', markersize=3, markerfacecolor='none', markeredgecolor='navy',linewidth=1.0, color='blue')
